...
import {
  ...
  ShieldCheck, ShieldAlert, CalendarRange, FolderArchive,
} from "lucide-react";
...
import {
  ...
  downloadRegistry, downloadArchive, cardKeys,
...
export default function AdminCardsPage() {
   ...
   *
   * ⚠️ NOT a print run. It increments archive_count rather than print_count —
   * a designer collecting material is a different act from a batch going to
   * the card printer, and one number cannot answer both questions.
   */
  const archive = useMutation({
    mutationFn: () => downloadArchive([...selectedCards], token),
    onSuccess: ({ included, skipped }) => {
      qc.invalidateQueries({ queryKey: cardKeys.registry });   // archive_count moved
      // ⚠️ Omissions are NAMED. A designer who receives 37 folders instead of
      // 40 must learn it here rather than by counting.
      if (skipped > 0) {
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--line)] bg-white">
        ...
        <div className="flex flex-wrap items-center gap-2.5 border-b border-[var(--line)] px-5 py-3">
          ...
          {/* ⚠️ Outline, not solid, and to the LEFT of Imprimer.
              Two solid buttons side by side read as equal choices; printing
              is the act this screen exists for, and the archive is the
              secondary path for whoever produces the physical card. */}
          <Button size="sm" variant="outline"
            disabled={selectedCards.size === 0 || archive.isPending}
            onClick={() => archive.mutate()}
            title="Photographies, codes QR et aperçus PDF, un dossier par carte">
            {archive.isPending
              ? <Loader2 className="h-3.5 w-3.5 animate-spin" />
              : <FolderArchive className="h-3.5 w-3.5" />}
            Archive {selectedCards.size > 0 && `(${selectedCards.size})`}
          </Button>
    ...
    <li className="flex items-center gap-4 px-5 py-3"
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[12px] text-[var(--slate)]">
          ...
          {/* Shown separately from printCount, because they mean different
              things: one is a card that went to the printer, the other is
              material a designer collected. */}
          {(card.archiveCount ?? 0) > 0 && (
            <>
              <span className="mx-1.5 opacity-40">·</span>
              archivée {card.archiveCount}×
            </>
          )}
        </p>
      </div>
