...
export default function ProfilePage() {
  ...
  const missing = useMemo(() => {
    /*
     * ⚠️ NOT AN EMPTY LIST. An empty list means "nothing is missing", and
     * that is a claim this function cannot make when it has no data — it
     * rendered as "0 éléments manquants" on a profile it had never seen.
     *
     * null means "unknown", and the screen says so rather than reassuring.
     */
  ...
  /*
   * ⚠️ A FAILED QUERY IS NOT A LOADED ONE.
   *
   * isLoading goes false on error too, and me.data stays undefined — so this
   * guard passed, the form mounted with its empty defaultValues, the reset
   * below never fired (it returns early on !me.data), and the gap list
   * reported nothing missing.
   *
   * The result was a profile screen showing blank fields and "0 éléments
   * manquants" — a page that looks like an empty profile rather than a
   * failure to load one. The person would have typed their name in again.
   */
  if (me.isError) {
    return (
      <div className="mx-auto max-w-5xl">
        <div className="rounded-2xl border border-[var(--red-500)]/40 bg-[var(--red-tint)] p-8 text-center sm:p-12">
          <AlertCircle className="mx-auto h-9 w-9 text-[var(--red-700)]" />
          <p className="mt-4 text-[15px] font-extrabold text-[var(--red-700)]">
            {t("loadFailedTitle")}
          </p>
          <p className="mx-auto mt-2 max-w-md text-[13.5px] leading-relaxed text-[var(--red-700)]">
            {t("loadFailedBody")}
          </p>
        ...
        <div className="relative z-10 p-5 sm:p-7">
          <div className="flex flex-wrap items-start justify-between gap-5">
            ...
            >
                ...
                /*
                 * ⚠️ THE THIRD STATE: THE PROFILE COULD NOT BE READ.
                 *
                 * It used to fall into the branch below and render "0" — a
                 * badge announcing a complete profile on a page that had
                 * loaded nothing. Two states could not express "unknown", so
                 * unknown was reported as success.
                 */
                <>
                  <p className="font-mono text-[28px] font-extrabold leading-none text-white/40">
                    —
                  </p>
                  <p className="mt-1.5 text-[9px] font-bold uppercase tracking-[0.14em] text-white/40">
                    {t("unknownCount")}
                  </p>
                </>
              ) : (
