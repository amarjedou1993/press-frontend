    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--paper)]">
        <span className="inline-flex items-center gap-1" aria-hidden="true">
          <i className="h-4 w-1.5 animate-pulse rounded-full bg-[var(--green-500)]" />
          <i className="h-4 w-1.5 animate-pulse rounded-full bg-[var(--gold-500)] [animation-delay:150ms]" />
          <i className="h-4 w-1.5 animate-pulse rounded-full bg-[var(--red-500)] [animation-delay:300ms]" />
        </span>
      </div>
    );
  }
    ...
    <SidebarProvider>
      ...
      <SidebarInset className="bg-[var(--paper)]">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-[var(--line)] bg-white/85 px-5 backdrop-blur">
          <SidebarTrigger
            className="text-[var(--slate)] hover:text-[var(--green-900)]"
            aria-label={t("toggleMenu")}
          >
            {/* rtl-flip: the icon depicts a panel ON THE LEFT. With the
                sidebar moved to the right in Arabic, an unflipped icon points
                at the wrong side of the screen. */}
            <PanelLeft className="rtl-flip h-4 w-4" />
          </SidebarTrigger>
      ...
      {/* Toasts appear at the TRAILING edge — away from where reading starts,
          so they never cover the beginning of a line. That is the right in a
          French page and the left in an Arabic one. */}
      <Toaster
        dir={arabic ? "rtl" : "ltr"}
        position={arabic ? "bottom-left" : "bottom-right"}
      />
    </SidebarProvider>
  );
}
