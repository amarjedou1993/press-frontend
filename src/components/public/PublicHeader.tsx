...
export async function PublicHeader() {
  ...
  /*
   * ───────────────────────────────────────────────────────────────────────
   * ⚠️ THE RIBBON ANNOUNCES THE CANDIDATURE, NOT WHICHEVER SESSION IS FIRST.
   *
   * fetchOpenSessions returns every RECEIVING session of either kind, ordered
   * by start date descending — so `sessions[0]` was simply the most recent.
   * Once renewals exist, a renewal opening after a candidature would take the
   * ribbon, and every visitor would read "les candidatures sont ouvertes"
   * about a cycle they may not enter.
   *
   * This is a PUBLIC header: its default reader is a stranger, and the
   * session that concerns them is the candidature. So that one is preferred
   * whenever it exists.
   *
   * ⚠️ A RENEWAL STILL GETS ITS OWN LINE when it is the only one open — a
   * holder whose invitation went to spam must not read "aucune session" while
   * their window runs and their card lapses.
   * ───────────────────────────────────────────────────────────────────────
   */
  const candidacy = sessions.find((s) => s.type === "CANDIDACY");
  const renewal = sessions.find((s) => s.type === "RENEWAL");
  const announced = candidacy ?? renewal;
  const open = !!announced;
  const announcingRenewal = !candidacy && !!renewal;
      ...
      >
        ...
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-4 gap-y-1 px-4 py-2 sm:px-6">
          <p className="flex items-center gap-2 text-[11.5px] font-semibold text-[var(--green-900)] sm:gap-2.5 sm:text-[12px]">
            ...
            />
            {announcingRenewal
              ? <RefreshCw className="h-3.5 w-3.5 flex-none text-[var(--green-700)]" />
              : <CalendarCheck2 className="h-3.5 w-3.5 flex-none text-[var(--green-700)]" />}
            {announced ? (
              /* The date is embedded in the phrase rather than appended:
                 Arabic and French put it in different places, and a
                 ...
                 ⚠️ Two phrasings, because the two sessions say different
                 things to a stranger. "Les candidatures sont ouvertes" is an
                 invitation; a renewal window is a notice to people who
                 already hold something. */
              th.rich(announcingRenewal ? "renewalOpen" : "sessionOpen", {
                date: fmtShort(announced.receivingEnd),
                b: (chunks) => <b className="font-extrabold">{chunks}</b>,
              })
            ) : (
              <span className="text-[var(--slate)]">{th("sessionClosed")}</span>
